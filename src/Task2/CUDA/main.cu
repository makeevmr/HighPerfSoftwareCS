...
[[nodiscard]] int arraySum(const int* host_arr_input, int* host_arr_ouput,
  ...
  cudaMemcpy(cuda_arr_input, host_arr_input, host_arr_input_size * sizeof(int),
...
int main(int argc, char* argv[]) {
  ...
  for (int _ = 0; _ < kIterations; ++_) {
    ...
    total_sum += arraySum(host_arr_input, host_arr_output, host_arr_input_size,
                          host_arr_output_size);
